...
TODO low: Compound the next three images into one figure with sub-images a, b and c
\begin{figure}[btph]
  \centering
...
As mentioned in the concept for RacecAR GO, the prototypical version of the game that is implemented within this thesis' scope will only implement a subset of all the world's makes and models, i. e. 10 different make-model-classes. The choice of the actual classes plays a role because of the following reasons:
\begin{description}
  \item[Control of multiplicity and ambiguity issues:] As defined in \emph{VR4}, the VMMR has to be able <em>"to cope with ambiguity and multiplicity issues"</em>. The choice of vehicle classes affects the discriminability of the different makes and models. Choosing 10 vehicle classes with high inter-class differences will increase the discrimination power of the VMMR. (TODO: Show examples and counter-examples. Good counter-example: VW Polo 9N3 and VW Golf V. Maybe just this example is enough). With equation TODO, this is explained in more detail. Let <formula>F_class_i</formula> be all the local features of <formula>class_i</formula>. Then <formula>E_class_i</formula> will be the intra-class error of the distances of all features from <formula>class_i</formula>. In other words, the nearer the local features lie to the center of <formula>F_class_i</formula>, the lesser the error.<br/>Furthermore, let <formula>E</formula> be the overall inter-class error among all centers of <formula>F_class_i</formula>. The greater this error, the more distance lies between the different class-features centers.<br/>So the target is to maximize <formula>E</formula> while minimizing all <formula>E_class_i, forall i</formula>.<br/>An optimal choice of vehicle classes will have the effect of a distribution of their local features as shown in figure TODO: show figure with near-optimal distribution in feature space (below). This also leads to a beneficial distribution of the dictionary codewords, because each cluster (and so the codeword) will be strongly related to a specific vehicle class. As a result, the histogram (the Global Representation) will encode one main vehicle class with each bin (see figure TODO: show figure with near-optimal per-class histogram bins (below)), which makes it easy for the classifier to classify by the histogram.<br/>This is achieved by training the classifier with all possible vehicle-class-subsets of size 10 (TODO number of limited vehicles) while maximizing the accuracy. For more details see TODO ref implementation chapter.
