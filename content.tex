...
The Advanced Modular Dictionary (AMD) scheme extends the Modular Dictionary scheme. To solve the problem of too few per-class-clusters, the per-class-clustering will be performed with a reasonable high number of clusters, whereas after the clustering, only the \emph{good} clusters are kept. To cope with a too large codeword pool inside the Modular Dictionary, the codewords of the per-class-dictionaries are clustered once more.
...
\begin{enumerate}
	\item The generation process starts the same way, the Modular Dictionary does: The local descriptors of all instances are gathered class-wise. In the figure, this step is shown exemplarily for class $c_1$. The cloud represents the local-descriptor-space while the blue dots mark the single local descriptors $f_1c1, ..., f_Nf$ (TODO: mark as numbered equation), where TODO.
	\item In this step, the local descriptors get clustered class-wise. There is also no difference to the proposed Modular Dictionary creation process up to now, except for the requested number of clusters. In contrast to the number evaluated best by TODO siddiqui, which was $29$, the number $k_c$ evaluated best for the AMD is about ten times as large (see (TODO impl chapter) for evaluation details). For each class $c_i$, this step produces $k_c$ clusters $omega_1ci, ..., omega_kcci$ TODO, shown as purple ellipsoids in the figure.
	\item TODO
	\item TODO
	\item Here, the results of the processes for the other classes come into account TODO expression? For each class, there is now a set of local descriptors $f_1 strich$ TODO
	\item TODO
\end{enumerate}

3. Aussortieren der schlechten Clusters. Formel mit Omega-Menge und lc1. Erklären, wie man auf $r_ci$ kommt: ratio * numInstances($c_i$)
4. Transformation der Clustercenter zu neuen feature descriptors f1'
5. Über die neuen Feature descriptors ALLER Klassen dann wiederum nochmal clustern zu k clustern. Though this will also cluster feature-clusters of different classes into one cluster (which was the inter-class ambiguity problem of the Single Dictionary), measures show that the performance increases for lower dictionary sizes (TODO hier ref. Implementierung, wo ich Messungen habe, die zeigen, dass zu große DictSize nicht mehr gut ist, und zu kleine per-class-DictSize auch nicht. In Impl darauf eingehen, dass es ein gutes Verhältnis braucht zw den beiden DictSizes und der Ratio).
6. Die k cluster centers werden die Dictionary codewords
...
TODO Das brauche ich dann nicht mehr, weil es alles in AMD steht.
