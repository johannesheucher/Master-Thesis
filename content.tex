...
where $l_{c_i} << k_c$ is the remaining number of good clusters and $r_{c_i} = N_{c_i} * t$.
	\item After having reduced the number of clusters, each cluster of local descriptors is turned into one single local descriptor, defined as the mean of this cluster. So each class is represented by a set of local descriptors
	\begin{equation}\label{eq:perClassCodewords}
	{f'}_1^{c_i},\dots,{f'}_{l_{c_i}}^{c_i} = mean(\Omega_1^{c_i}),\dots,mean(\Omega_{l_{c_i}}^{c_i})
	\end{equation}
	\item Here, the results of the processes for the other classes are taken into account. For each class, there is a set of local descriptors $f'_{c_i}$ now, which have been filtered to be expressive for their class. Analogously to step 1, which gathers the local descriptors $f$ of all RoIs class-wise, the new local descriptors $f'$ are gathered over all classes, resulting into a set
...
For the Modular Dictionary, the set from equation \ref{eq:allDescriptors} already forms the codewords of the dictionary. But to cope with the large amount of resulting per-class descriptors $f'$, a further clustering process is deployed. Im Gegensatz zum Reduction-Process from equation \ref{eq:clustersReduction} ist es hier gut, wenn ein Cluster wenige local descriptors enthält, da das bedeutet, dass das resultierende codeword nur wenige (ideally genau eine) Klasse kodiert. A cluster containing a local descriptor of each class is useless, because the resulting codeword would encode each class likewise. So an amplitude to that codeword by a Global Representation does not tell anything about the class-belonging. To control the clustering process, wird ein weiterer Degree of Freedom (the desired number $k$ of resulting clusters) spendiert, um zu ermitteln, ob sich ein weiteres Clustering lohnt. Possibly, it doesn't and the evaluated best $k$ just equals the number of inputted descriptors TODO ref. equation mit f-strich-en.
	\item TODO Die k cluster centers werden die Dictionary codewords
\end{enumerate}
